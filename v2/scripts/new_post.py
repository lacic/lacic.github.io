...
Creates a new post folder under v2/src/content/updates/ with frontmatter that
...
def create_post(
    ...
    # Resolved from the script's location, so running this from the repo root
    # and from v2/ produce the same result.
    ...
    if was_taken:
        ...
        print(f'     uv run v2/scripts/new_post.py "{title}" --date {day.isoformat()}')
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            '  uv run v2/scripts/new_post.py "Why our A/B test disagreed with the offline evaluation"\n'
            '  uv run v2/scripts/new_post.py "Ranking latency, part three" \\\n'
    ...
    if not args.title.strip():
        ...
        print('   Usage: uv run v2/scripts/new_post.py "Your post title"')
        sys.exit(1)
